...
def dy_reptile():
    ...
    for comment in comments:
        ...
        db.dycomments.insert_one(document)
    #db.comments.insert_many({"url":url},{"comments":comments})
    result = {"url":url,"id":video_id,"title":title}
    print(db.dycomments.find({url : url}))
    return result
    #if want to search:db.comments.find({url:XXX})